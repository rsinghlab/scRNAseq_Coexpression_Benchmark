...
def load4DiffSetting(metric_dir_path, data_name, sim_name, num_genes):
    raw_metric_list = [each for each in os.listdir(metric_dir_path) if (data_name in each) and (sim_name in each) and ("process-metric" not in each) and (num_genes in each)]
    processed_metric_list = [each for each in os.listdir(metric_dir_path) if (data_name in each) and (sim_name in each) and ("process-metric" in each) and (num_genes in each)]
    metric_dict = {}
    ...
    metric_dict["baseline"] = {"-".join(each.split("-")[:3]): _randomBaseline(each) for each in raw_metric_list}
    return metric_dict
...
if __name__ == '__main__':
    metric_dir_path = "../res/logs/evaluation/high_dim/"
