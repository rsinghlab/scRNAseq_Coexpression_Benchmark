# Description: Benchmark for SOTA graphical models on SERGIO simulation data.
# Author: Jiaqi Zhang
...
if (FALSE) {
  ...
  # data_filename <- sprintf("./data/pseudo_bulk/%s-%s-data_mat.csv", data_name, sim_name)
  data_filename <- sprintf("./data/SERGIO_simulation_all/%s-%ssparsity.csv", data_name, sparsity)
  net_filename <- sprintf("./data/SERGIO_simulation_all/true_network.csv")
  # net_filename <- sprintf("./data/SERGIO_simulation_all/100gene-true_network.csv")
  ...
  # write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%ssparsity-%s-metrics.csv", metric_save_path, data_name, sparsity, need_preprocess_str))
  # saveRDS(model_predictions, sprintf("%s/%s-%ssparsity-%s-predictions.rds", pred_save_path, data_name, sparsity, need_preprocess_str))
}