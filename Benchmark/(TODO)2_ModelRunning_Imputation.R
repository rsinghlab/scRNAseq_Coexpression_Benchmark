# Description: Benchmark for SOTA graphical models on imputated simulation data.
# Author: Jiaqi Zhang
...
if (TRUE) {
  ...
  print(sprintf("[ %s | %s ] Loading data %s...", data_name, imputation_name, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/imputed_simulation/%s-%s-%s-data_mat.csv", data_name, sim_name, imputation_name)
  ...
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s-%s%s-metrics.csv", metric_save_path, data_name, sim_name, imputation_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s-%s-%s%s-predictions.rds", pred_save_path, data_name, sim_name, imputation_name, need_preprocess_str))
  saveRDS(model_conf_mat, sprintf("%s/%s-%s-%s%s-sign_conf_mats.rds", pred_save_path, data_name, sim_name, imputation_name, need_preprocess_str))
}


# Model running on imputed SERGIO data
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  ...
  print(paste(rep("*", 70), collapse = ""))
  print(sprintf("[ %s | %s ] Loading data %s...", data_name, imputation_name, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/SERGIO_imputation/%s-%s-data_mat.csv", data_name, imputation_name)
  ...
  saveRDS(model_predictions, sprintf("%s/%s-%s-predictions.rds", pred_save_path, data_name, imputation_name))
}
