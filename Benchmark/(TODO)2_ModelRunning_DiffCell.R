...
if (TRUE) {
  ...
  parser$add_argument("-c", "--cell_ratio", default = '0.1cell', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "1", help = "Need preprocessing.")
  ...
  print(sprintf("[ %s | %s ] Loading data %s...", data_name, cell_ratio, ifelse(need_preprocess, "(pre-process)", "")))
  # data_filename <- sprintf("./data/pseudo_bulk/%s-%s-data_mat.csv", data_name, sim_name)
  data_filename <- sprintf("./data/diff_cell/%s-%s-NORTA-data_mat.csv", data_name, cell_ratio)
  net_filename <- sprintf("./data/diff_cell/%s-net_mat.csv", data_name)
  ...
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s%s-metrics.csv", metric_save_path, data_name, cell_ratio, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s-%s%s-predictions.rds", pred_save_path, data_name, cell_ratio, need_preprocess_str))
}