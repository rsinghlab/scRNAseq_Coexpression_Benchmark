# Description: Benchmark for SOTA graphical models on real scRNA-seq data.
# Author: Jiaqi Zhang
...
estimateNet <- function(cov_mat, data, model_args, type) {
  if (type == "thresholding") {
    ...
    # -----
    # model_args$X <- cov_mat
    # model_args$method <-
    # est_network <- do.call("PreEst.glasso", model_args)$X
    est_network <- PreEst.glasso(X = data, method=list(type="fixed", param=model_args$rho), parallel=TRUE)$C
    est_network[abs(est_network) < 1e-3] <- 0.0
  } else {
...
runModel <- function(name, data, num_steps) {
  ...
  for (t in 1:length(par_space)) {
    ...
    est_network_list[[t]] <- estimateNet(cov_mat, data=data, model_args = step_pars, type = estimate_type)
  }
...
all_models_list <- c("pearson", "spearman", "glasso", "scLink", "GENIE3", "scDesign2", "minet")
all_models_list <- c("PIDC", "pearson", "spearman", "glasso", "scLink", "GENIE3", "scDesign2", "minet")
# all_models_list <- c("pearson", "spearman")

# Model running on true data
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-d", "--data_name", default = 'pbmc1-inDrops-1000hvg', help = "scRNA-seq data name.")
...
# if(FALSE){
#   net_path <- "./data/experimental/mouse_cortex/processed/network/"
#   # net_path <- "./data/experimental/PBMC/processed/network/"
#   file_list <- list.files(net_path, pattern=".+STRING.+")
#   for (each in file_list){
#     if (grepl("full", each, fixed = TRUE)){}
#     else{
#     network <- readMM(sprintf("%s/%s", net_path, each))
#   network <- as.matrix(network)
#   network <- apply(network, c(1, 2), as.numeric)
#     triu_net <- network[upper.tri(network, diag = FALSE)]
#     print(sprintf("[ %s ] # edges = %d", each, sum(triu_net)))
#     }
#   }
# }
