...
suppressPackageStartupMessages({
  ...
  # library(scran) # for Scran
  # library(Seurat)
  # library(scuttle) # for scran
})
...
all_models_list <- c("pearson", "spearman", "glasso", "scLink", "GENIE3", "PIDC", "minet", "scDesign2")
# all_models_list <- c("scDesign2")
...
if (FALSE) {
  ...
  all_model_metrics$FDR <- all_model_metrics$FP / (all_model_metrics$FP + all_model_metrics$TP)
  options(warn = defaultW)
  # Save data
  metric_save_path <- "./res/logs/evaluation/other_norm"
  pred_save_path <- "./res/prediction/other_norm/"
...
# Data stats of normalized NORTA simualted data
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  ...
  parser$add_argument("-p", "--norm_type", default = "sctransform", help = "Type of normalization.")
  args <- parser$parse_args()
  data_name <- args$data_name
  simulation_name <- args$simulation_name
  num_steps <- as.integer(args$num_steps)
  ...
  data <- normData(data, norm_type)
  # -----
  sign_data <- sign(abs(data))
  gene_sparsity <- colMeans(sign_data)
  avg_sparsity <- mean(gene_sparsity)
  std_sparsity <- sd(gene_sparsity)
  print(sprintf("%f (%f)", avg_sparsity, std_sparsity))
}


# Data stats of normalized SERGIO simualted data
if (TRUE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
