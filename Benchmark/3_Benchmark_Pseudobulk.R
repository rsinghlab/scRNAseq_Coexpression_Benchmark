# Description: Benchmark of exisiting models on pseudo-bulks of simulations.
...
if (TRUE) {
  ...
  data_filename <- sprintf("./data/pre-processing/pseudo_bulk/NORTA/%s-%s-data_mat.csv", data_name, sim_name)
  ...
  # Save data
  metric_save_path <- "./res/pre-processing/pseudo_bulk/NORTA/"
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s-metrics.csv", metric_save_path, data_name, sim_name))
}
...
if (TRUE) {
  ...
  print(paste(rep("*", 70), collapse = ""))
  print(sprintf("[ %s ] Loading data...", data_name))
  data_filename <- sprintf("./data/pre-processing/pseudo_bulk/SERGIO/%s.csv", data_name)
  net_filename <- sprintf("./data/simulation/SERGIO/100gene-true_network.csv", data_name)
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  print(sprintf("Data shape : #cells=%d, #genes=%d", dim(data)[1], dim(data)[2]))
  ...
  options(warn = defaultW)
  # Save data
  metric_save_path <- "./res/pre-processing/pseudo_bulk/SERGIO/"
