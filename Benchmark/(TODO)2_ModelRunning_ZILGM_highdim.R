...
if (FALSE) {
  ...
  parser$add_argument("-d", "--data_name", default = 'pbmc1-inDrops-500hvg', help = "Simulation data name.")
  ...
  metric_save_path <- "./res/logs/evaluation/ZILGM_model_highdim/"
  pred_save_path <- "./res/prediction/ZILGM_model_highdim/"
...
if (TRUE) {
  ...
  pred_save_path <- "./res/prediction/SERGIO_400genes/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s%s-ZILGM-metrics.csv", metric_save_path, data_name, simulation_name, need_preprocess_str))
  # saveRDS(model_predictions, sprintf("%s/%s-%s%s-predictions.rds", pred_save_path, data_name, simulation_name, need_preprocess_str))
}