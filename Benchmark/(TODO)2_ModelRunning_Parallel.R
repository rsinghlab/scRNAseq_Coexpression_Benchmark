...
netEdgeNum <- function (network){
  ...
  network_edge <- sign(abs(network))
  triu_net <- network_edge[upper.tri(network_edge, diag = FALSE)]
...
if (FALSE) {
  ...
  parser$add_argument("-n", "--num_steps", default = '5', help = "Number of steps.")
  ...
  data_filename <- sprintf("./data/high_dim/%s-%s-data_mat.csv", data_name, simulation_name)
...
if (TRUE) {
  ...
  net_filename <- sprintf("./data/SERGIO_simulation_all/400gene-true_network.csv")
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  print(sprintf("Data shape : #cells=%d, #genes=%d", dim(data)[1], dim(data)[2]))
  ...
  options(warn = defaultW)
  # # Save data
  metric_save_path <- "./res/logs/evaluation/SERGIO_400genes"
  # pred_save_path <- "./res/prediction/high_dim/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s%s-PIDC-metrics.csv", metric_save_path, data_name, simulation_name, need_preprocess_str))
  # saveRDS(model_predictions, sprintf("%s/%s-%s%s-predictions.rds", pred_save_path, data_name, simulation_name, need_preprocess_str))
}
