# Description: Benchmark for SOTA graphical models on pseudo-bulk simulation data.
# Author: Jiaqi Zhang
...
if (FALSE) {
  ...
  parser$add_argument("-d", "--data_name", default = 'Cortex1-10xChromium-50hvg', help = "scRNA-seq data name.")
  ...
  metric_save_path <- "./res/logs/evaluation/pseudo_bulk_sum/"
  pred_save_path <- "./res/prediction/pseudo_bulk_sum/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s-%s-metrics.csv", metric_save_path, data_name, sim_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s-%s-%s-predictions.rds", pred_save_path, data_name, sim_name, need_preprocess_str))
}
...
if (FALSE) {
  ...
  saveRDS(model_predictions, sprintf("%s/%s--predictions.rds", pred_save_path, data_name))
}