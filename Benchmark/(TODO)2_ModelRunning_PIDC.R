...
loadRealData <- function(data_filename, net_filename) {
  data <- read.csv(data_filename, header = 1, row.names = 1)
  network <- readMM(net_filename)
  network <- as.matrix(network)
  network <- apply(network, c(1, 2), as.numeric)
  return(list(data = as.matrix(data), network = network))
}
...
if (FALSE) {
  ...
  metric_save_path <- "./res/logs/evaluation/SERGIO_simulation_all"
  pred_save_path <- "./res/prediction/SERGIO_simulation_all/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s%s-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str))
}

# ----------------------------------------

# Model running on NORTA pseudo-bulk
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-d", "--data_name", default = 'pbmc1-inDrops-50hvg', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
  ...
  data_filename <- sprintf("./data/pseudo_bulk/%s-NORTA-data_mat.csv", data_name)
  ...
  # Save data
  # a <- apply(all_model_metrics, 2, as.character)
  metric_save_path <- "./res/logs/evaluation/pseudo_bulk"
  pred_save_path <- "./res/prediction/pseudo_bulk/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-NORTA-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s%s-NORTA-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str))
}


# Model running on SERGIO pseudo-bulk
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  ...
  net_filename <- sprintf("./data/SERGIO_simulation_all/true_network.csv", data_name)
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  # Pre-processing
  ...
  # Save data
  # a <- apply(all_model_metrics, 2, as.character)
  metric_save_path <- "./res/logs/evaluation/SERGIO_pseudobulk"
  pred_save_path <- "./res/prediction/SERGIO_pseudobulk/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s%s-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str))
}
...
# Model running on NORTA imputation
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-s", "--simulation_name", default = 'NORTA-SAVER', help = "scRNA-seq data name.")
  ...
  print(paste(rep("*", 70), collapse = ""))
  print(sprintf("[ %s | %s ] Loading data %s...", data_name, simulation_name, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/imputed_simulation/%s-%s-data_mat.csv", data_name, simulation_name)
  net_filename <- sprintf("./data/simulated/new/%s-net_mat.csv", data_name)
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  ...
  # Save data
  # a <- apply(all_model_metrics, 2, as.character)
  metric_save_path <- "./res/logs/evaluation/imputed_simulation"
  pred_save_path <- "./res/prediction/imputed_simulation/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-%s-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str, simulation_name))
  saveRDS(model_predictions, sprintf("%s/%s%s-%s-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str, simulation_name))
}


# Model running on SERGIO imputation
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-d", "--data_name", default = '100gene-9groups-1-SAVER', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
  ...
  print(paste(rep("*", 70), collapse = ""))
  print(sprintf("[ %s ] Loading data %s...", data_name, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/SERGIO_imputation/%s-data_mat.csv", data_name)
  net_filename <- sprintf("./data/SERGIO_simulation_all/true_network.csv")
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  ...
  # Save data
  # a <- apply(all_model_metrics, 2, as.character)
  metric_save_path <- "./res/logs/evaluation/SERGIO_imputation"
  pred_save_path <- "./res/prediction/SERGIO_imputation/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s%s-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str))
}
...
# Model running on different cell num
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  ...
  parser$add_argument("-d", "--data_name", default = 'Cortex1-10xChromium-100hvg-2.0cell', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
...
if (FALSE) {
  ...
  parser <- ArgumentParser()
  parser$add_argument("-s", "--simulation_name", default = 'NORTA', help = "scRNA-seq data name.")
  parser$add_argument("-d", "--data_name", default = 'Cortex1-10xChromium-100hvg-power', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
...
if (FALSE) {
  ...
  parser <- ArgumentParser()
  parser$add_argument("-s", "--simulation_name", default = 'NORTA', help = "scRNA-seq data name.")
  parser$add_argument("-d", "--data_name", default = '100gene-hub-full', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
...
# Model running on scLink simulation
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-s", "--simulation_name", default = 'scLink', help = "scRNA-seq data name.")
  parser$add_argument("-d", "--data_name", default = '100hvg-scLink_0.07rho', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
...
# Model running on ZILGM simulation
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-s", "--simulation_name", default = 'ZILGM', help = "scRNA-seq data name.")
  parser$add_argument("-d", "--data_name", default = '50hvg-ZILGM_0.8pi', help = "scRNA-seq data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  data_name <- args$data_name
...
# Model running on high-dim data
if (FALSE) {
  defaultW <- getOption("warn")
  options(warn = -1)
  # Parameters
  ...
  parser$add_argument("-d", "--data_name", default = 'pbmc2-inDrops-500hvg', help = "Simulation data name.")
  parser$add_argument("-n", "--num_steps", default = '100', help = "Number of steps.")
  parser$add_argument("-p", "--need_preprocessing", default = "0", help = "Need preprocessing.")
  args <- parser$parse_args()
  simulation_name <- args$simulation_model
  ...
  print(paste(rep("*", 70), collapse = ""))
  print(sprintf("[ %s | %s ] Loading data %s...", data_name, simulation_name, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/high_dim/%s-NORTA-data_mat.csv", data_name)
  net_filename <- sprintf("./data/high_dim/%s-net_mat.csv", data_name)
  data_obj <- loadSimulation(data_filename, net_filename)
  data <- data_obj$data
  true_network <- data_obj$network
  # Pre-processing
  ...
  # Save data
  # a <- apply(all_model_metrics, 2, as.character)
  metric_save_path <- "./res/logs/evaluation/high_dim"
  pred_save_path <- "./res/prediction/high_dim/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s%s-NORTA-PIDC-metrics.csv", metric_save_path, data_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s%s-NORTA-PIDC-predictions.rds", pred_save_path, data_name, need_preprocess_str))
}
...
if (FALSE) {
  ...
  # Parameters
  parser <- ArgumentParser()
  parser$add_argument("-d", "--data_name", default = 'Cortex1-Smart_seq2-1000hvg', help = "scRNA-seq data name.")
  ...
  data_dir_path <- ifelse(
    grepl("Cortex", data_name, fixed = TRUE),
    # "./data/experimental/mouse_cortex/processed/expr/",
    "/gpfs/data/rsingh47/jzhan322/Coexpression_Benchmark/data/experimental/mouse_cortex/processed/expr/",
    # "./data/experimental/PBMC/processed/expr/"
    "/gpfs/data/rsingh47/jzhan322/Coexpression_Benchmark/data/experimental/PBMC/processed/expr/"
  )
  net_dir_path <- ifelse(
    grepl("Cortex", data_name, fixed = TRUE),
    # "./data/experimental/mouse_cortex/processed/network/",
    "/gpfs/data/rsingh47/jzhan322/Coexpression_Benchmark/data/experimental/mouse_cortex/processed/network/",
    # "./data/experimental/PBMC/processed/network/"
    "/gpfs/data/rsingh47/jzhan322/Coexpression_Benchmark/data/experimental/PBMC/processed/network/"
  )
  ...
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s%s-PIDC-metrics.csv", metric_save_path, data_name, true_net_name, need_preprocess_str))
  saveRDS(model_predictions, sprintf("%s/%s-%s%s-PIDC-predictions.rds", pred_save_path, data_name, true_net_name, need_preprocess_str))
}