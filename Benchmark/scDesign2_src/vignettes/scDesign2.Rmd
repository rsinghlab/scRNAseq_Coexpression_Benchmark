...
stats_test <- get_stats(testcount_sel, 'SERGIO_simulation_all')
...
stats_dat$group <- factor(stats_dat$group, levels = c('training', 'SERGIO_simulation_all', 'scDesign2'))
...
group_list <- c('training data', 'SERGIO_simulation_all data', 'scDesign2')
...
get_dim_red_dat <- function(dim_red_result){
  ...
  dim_red_dat$panels <- factor(c(rep('training data', ncol(traincount_sel)),
                                 rep('SERGIO_simulation_all data', ncol(testcount_sel)),
                                 rep('scDesign2', ncol(sim_count_copula))),
                               levels = c('training data', 'SERGIO_simulation_all data', 'scDesign2'))
  return(dim_red_dat)
}
...
pca_plot <- get_dim_red_plot(pca_dat, c('SERGIO_simulation_all data PC 1', 'SERGIO_simulation_all data PC 2'))
...
```{r fit model from original data,eval=FALSE}
cell_type_sel <- c("Stem", "Goblet", "Tuft", "TA.Early",
                   "Enterocyte.Progenitor", "Enterocyte.Progenitor.Early")
...
```{r simulate count scenario 1,eval=FALSE}
...
```{r simulate count scenario 2,eval=FALSE}
set.seed(1)
...
```{r dim reduction scenario 1,eval=FALSE}
...
```{r Seurat Louvain clustering scenario 1,eval=FALSE}
library(Seurat)
library(mclust)
library(aricode)
...
```{r SC3 clustering scenario 1,eval=FALSE}
...
```{r load exp design scenario 1 summary,include=FALSE}
...
```{r exp design clustering scenario 1 plots,fig.width=7.2,fig.height=8,fig.align='center'}
