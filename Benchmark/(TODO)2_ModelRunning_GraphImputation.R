# Description: Benchmark for SOTA graphical models on (graph-based) imputated simulation data.
# Author: Jiaqi Zhang
...
if (TRUE) {
  ...
  print(sprintf("[ %s | %s | %s ] Loading data %s...", data_name, imputation_name, removed_edge, ifelse(need_preprocess, "(pre-process)", "")))
  data_filename <- sprintf("./data/graph_imputed_simulation/%s-%s-%s-n%s-data_mat.csv", data_name, sim_name, imputation_name, removed_edge)
  ...
  for (i in 1:range(length(all_models_list))) {
    ...
    metric_table["RemoveNum"] <- removed_edge
  ...
  # Save data
  metric_save_path <- "./res/logs/evaluation/graph_imputed_simulation/"
  pred_save_path <- "./res/prediction/graph_imputed_simulation/"
  need_preprocess_str <- ifelse(need_preprocess, "-process", "")
  write.csv(apply(all_model_metrics, 2, as.character), sprintf("%s/%s-%s-%s%s-n%s-metrics.csv", metric_save_path, data_name, sim_name, imputation_name, need_preprocess_str, removed_edge))
  saveRDS(model_predictions, sprintf("%s/%s-%s-%s%s-n%s-predictions.rds", pred_save_path, data_name, sim_name, imputation_name, need_preprocess_str, removed_edge))
}
